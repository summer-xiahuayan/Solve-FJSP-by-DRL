...
class Env(gym.Env):
    ...
    def state_obs(self) -> List[float]:
        ...
        for job in np.arange(self.num_jobs):
            ...
            obs.append(next_task_in_job.runtimes)
            obs.append(next_task_in_job.runtime / self.max_runtime)
            #obs.append(next_task_in_job.runtime)
    ...
    def choose_machine(self, task: Task) -> int:
        ...
        """
        #计算最早结束完工的时间，因为同一工序每个机器的加工时间不同
        possible_machines = task.machines
        runtimes=[x for x in task.runtimes]
        ends_of_finish_if_selscted=[self.ends_of_machine_occupancies[i]+runtimes[i] for i in range(len(possible_machines)) ]
        machine_times = np.where(possible_machines,
                                 ends_of_finish_if_selscted,
    ...
    def execute_action(self, job_id: int, task: Task, machine_id: int) -> None:
        ...
        """
       #  将选择的机器时间赋予runtime
        task.runtime=task.runtimes[machine_id]
    ...
    def render(self, mode='image'):
        ...
        if mode == 'human':
            ...
            return GanttChartPlotter.get_gantt_chart_image(self.tasks,return_image=True)
        else:
